...
export default function GoTurns(){
    ...
    const updateCOVID = async (e) => { //e es un evento
        ...
        if(turn!== ''){
            ...
            emailjs.sendForm(process.env.REACT_APP_EMAILJS_SERVICE_ID, process.env.REACT_APP_EMAILJS_TEMPLATE_TURN, process.env.REACT_APP_ADMIN_EMAIL, process.env.REACT_APP_EMAILJS_PUBLIC_KEY)
                .then((result) => {
    ...
    const updateYellow = async (e) => { //e es un evento
        ...
        if(turn2!== ''){
            ...
            MySwal.fire("Turno asignado");
            emailjs.sendForm(process.env.REACT_APP_EMAILJS_SERVICE_ID, process.env.REACT_APP_EMAILJS_TEMPLATE_TURN, inputRefEmail2.current.value, process.env.REACT_APP_EMAILJS_PUBLIC_KEY)
                .then((result) => {
                    console.log(result.text);
                }, (error) => {
                    console.log(error.text);
